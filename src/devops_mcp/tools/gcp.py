...
def register(mcp: Any, audit: Callable[[str, dict], None]) -> None:
    try:
                ...
                entries.append(
                    {
                        ...
                        "resource_type": (
                            entry.resource.type if entry.resource else None
                        ),
        ...
        def gcp_get_gke_cluster_status(
            ...
            pid = project or default_project
            audit(
                "gcp_get_gke_cluster_status",
                {"project": pid, "cluster": cluster_name, "location": location},
            )
        ...
        def gcp_get_cloud_run_status(
            ...
            pid = project or default_project
            audit(
                "gcp_get_cloud_run_status",
                {"project": pid, "service": service_name, "region": region},
            )
        ...
        def gcp_get_logs(  # type: ignore[misc]
            ...
            return json.dumps(
                [
                    {
                        "timestamp": "2024-01-15T09:08:00+00:00",
                        "severity": "ERROR",
                        "resource_type": "k8s_container",
                        "payload": "OOMKilled: container exceeded memory limit",
                    },
                    {
                        "timestamp": "2024-01-15T09:08:02+00:00",
                        "severity": "WARNING",
                        "resource_type": "k8s_container",
                        "payload": "Readiness probe failed: connection refused",
                    },
                    {
                        "timestamp": "2024-01-15T09:09:15+00:00",
                        "severity": "ERROR",
                        "resource_type": "cloudsql_database",
                        "payload": "Too many connections: max_connections=100 reached",
                    },
                ],
                indent=2,
            )
        ...
        def gcp_get_metric(  # type: ignore[misc]
            ...
            return json.dumps(
                [
                    ...
                    {
                        "timestamp": "2024-01-15T09:10:00+00:00",
                        "labels": {"instance_name": "web-01"},
                        "value": 0.94,
                    },
                ],
        ...
        def gcp_get_cloud_run_status(  # type: ignore[misc]
            ...
            audit(
                "gcp_get_cloud_run_status", {"service": service_name, "region": region}
            )
            return json.dumps(
                {
                    "service": service_name,
                    ...
                    "traffic": [
                        {
                            "revision": f"{service_name}-00042-xyz",
                            "percent": 100,
                            "tag": "",
                        },
                    ],
