...
def register(mcp: Any, audit: Callable[[str, dict], None]) -> None:
    ...
    if not instance:
        ...
        def snow_list_incidents(  # type: ignore[misc]
            ...
            return json.dumps([
                {"number": "INC0045231", "short_description": "API gateway 502 errors spiking", "priority": "1 - Critical", "state": "In Progress", "assigned_to": "oncall-sre", "opened_at": "2024-01-15T08:52:00Z"},
                {"number": "INC0045198", "short_description": "Build pipeline stuck: artifact upload timeout", "priority": "2 - High", "state": "New", "assigned_to": "", "opened_at": "2024-01-15T07:30:00Z"},
                {"number": "INC0045102", "short_description": "K8s node NotReady: node-4 disk pressure", "priority": "2 - High", "state": "In Progress", "assigned_to": "platform-team", "opened_at": "2024-01-15T06:10:00Z"},
            ], indent=2)
        ...
        def snow_get_change_requests(  # type: ignore[misc]
            ...
            return json.dumps([
                {"number": "CHG0012481", "short_description": "Upgrade K8s 1.29 to 1.30 on prod cluster", "state": "Scheduled", "risk": "Moderate", "start_date": "2024-01-20T02:00:00Z", "end_date": "2024-01-20T05:00:00Z"},
                {"number": "CHG0012450", "short_description": "MSK Kafka broker scale-out: 6 to 9 brokers", "state": "Scheduled", "risk": "Low", "start_date": "2024-01-18T22:00:00Z", "end_date": "2024-01-18T23:30:00Z"},
            ], indent=2)
    ...
    def snow_list_incidents(
        ...
        audit("snow_list_incidents", {"state": state, "limit": limit, "assigned_to": assigned_to})
        ...
        resp = requests.get(f"{base_url}/table/incident", auth=auth, headers=headers, params=params, timeout=15)
    ...
    def snow_get_change_requests(
        ...
        resp = requests.get(f"{base_url}/table/change_request", auth=auth, headers=headers, params=params, timeout=15)
        resp.raise_for_status()
        records = resp.json().get("result", [])
        return json.dumps([
            {
