...
def register(mcp: Any, audit: Callable[[str, dict], None]) -> None:
    try:
        ...
        def azure_get_monitor_logs(
            ...
            audit(
                "azure_get_monitor_logs", {"workspace_id": workspace_id, "query": query}
            )
            ...
            return json.dumps(
                {"error": "Query failed", "details": str(response.partial_error)}
            )
        ...
        def azure_get_metric(
            ...
            audit(
                "azure_get_metric",
                {"resource_uri": resource_uri, "metric_name": metric_name},
            )
            ...
            for metric in response.metrics:
                for ts in metric.timeseries:
                    for point in ts.data:
                        ...
                        if val is not None:
                            results.append(
                                {"timestamp": str(point.timestamp), aggregation: val}
                            )
            ...
            return json.dumps(
                {
                    ...
                    "power_state": (
                        cluster.power_state.code if cluster.power_state else "Unknown"
                    ),
                    "agent_pools": pools,
                },
                indent=2,
        ...
        def azure_get_monitor_logs(  # type: ignore[misc]
            ...
            return json.dumps(
                [
                    {
                        "TimeGenerated": "2024-01-15T09:08:12Z",
                        "Level": "Error",
                        "Message": "Connection pool exhausted",
                        "Computer": "aks-node-001",
                    },
                    {
                        "TimeGenerated": "2024-01-15T09:08:14Z",
                        "Level": "Warning",
                        "Message": "Retry attempt 1 of 3",
                        "Computer": "aks-node-001",
                    },
                    {
                        "TimeGenerated": "2024-01-15T09:09:01Z",
                        "Level": "Error",
                        "Message": "Request timeout after 30s",
                        "Computer": "aks-node-002",
                    },
                ],
                indent=2,
            )
