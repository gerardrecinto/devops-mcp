...
def register(mcp: Any, audit: Callable[[str, dict], None]) -> None:
    try:
        ...
        def slack_post_message(  # type: ignore[misc]
            ...
            audit("slack_post_message", {"channel": channel})
            return json.dumps(
                {"ok": True, "ts": "1705305600.123456", "channel": channel}
            )
        ...
        def slack_get_channel_history(  # type: ignore[misc]
            ...
            return json.dumps(
                [
                    {
                        "ts": "1705305480.000100",
                        "user": "U01234ABC",
                        "text": "prod api latency is spiking, looking into it",
                    },
                    {
                        "ts": "1705305540.000200",
                        "user": "U05678DEF",
                        "text": "confirmed, cloudwatch shows db connections at 97%",
                    },
                    {
                        "ts": "1705305600.000300",
                        "user": "U01234ABC",
                        "text": "db-prod-0 OOMKilled, restarting now",
                    },
                ],
                indent=2,
            )
                ...
                [
                    {
                        ...
                        "ts": "1705305480.000100",
                        "user": "oncall-bot",
                        "text": f"[ALERT] {query} triggered P1 threshold",
                        "permalink": "https://slack.com/archives/C001/p1705305480000100",
                    },
                    {
                        "channel": "platform",
                        "ts": "1704700000.000200",
                        "user": "gerard",
                        "text": f"investigated {query} last week, fix is in deploy pipeline",
                        "permalink": "https://slack.com/archives/C002/p1704700000000200",
                    },
                ],
                indent=2,
            )
